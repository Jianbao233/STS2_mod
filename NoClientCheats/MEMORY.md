...
>
> **最新验证结果 (2026-04-08 13:55)**：用户反馈“一次作弊后，后续正常流程又莫名其妙弹两次”，并补充“重开很多把后，新进一把刚加载完就双弹窗”。最新日志确认这是两条误判链叠加：1) `OnReceivePlayerChoice` 在地图/奖励阶段会对同一个**弱 payload**（只有 `indexes`，没有 `deckCards/canonicalCards`）重复进入两次，之前直接累加 `_choiceCallCount`，会把一次正常选牌误记成 `2`；随后 `SyncReceived` 把正常 `11 -> 11 / 10 -> 11` 当成 `reward_multi_select`。2) 新局初始化/角色切换时，主机会先看到旧基线，再收到“新角色起始卡组 + Ascender's Bane”，之前通用 `add_cards(count=1 allowed=0)` 会把这个开局同步误报成作弊。**最新方案（2026-04-08，已构建成功）**：在 `DeckSyncPatches.cs` 中新增“弱 `PlayerChoice` 重复消息去重”，相同签名 2 秒内不再重复累计 `choiceCallCount`；同时新增 `startup sync warmup`，在首个同步、角色切换、起始卡组稳定阶段只重建基线，不走 `add_cards / reward_multi_select` 检测；另外 detect-only 的 `SyncReceived` 首次告警后会立即刷新通知抑制并清掉 choice 跟踪，避免后续重复弹窗。**下一步**：重点复测“新开一把刚加载进游戏是否还会立刻双弹窗”，以及“一次真实作弊后，后续正常推进是否还会再弹旧作弊提示”。
...
| `CheatNotification.cs` | `CanvasLayer` 红色通知弹窗，最多同时 4 个，带 Tween 渐隐；区分 blocked / detected 两类标题 |
...
| `CheatLocHelper.cs` | 反射 `LocString`，汉化角色名/遗物名/卡牌/指令；可把 `deck:` / `game_action:` / `ui_exploit:` 机器串转换为人类可读提示 |
| `HarmonyPatches/` | 各补丁类分目录存放 |
| `ClientDiagnosticPatches.cs` | 客机回滚修复模块：登记 NCC 回滚快照的原始目标玩家，并在 `SyncWithSerializedPlayer` 前重定向到正确玩家 |
| `DeckSyncPatches.cs / RewardObtainedMessagePostfix` | 奖励牌同步基线修复：主机收到 `RewardObtainedMessage(Card)` 后优先刷新该远程玩家的 NCC 基线；若地图阶段拿不到 live `Player`，则回退为仅推进 `_lastSyncState / _lastSyncDeckSize` 的合法 `+1` 基线，并对重复奖励消息去重，避免奖励后下一条 `SyncReceived` 被误判为 `add_cards` |
| `DeckSyncPatches.cs / _TrySendRollbackToPeer` | host-only 回滚发送策略：默认不再向未证明具备 NCC 客机补丁能力的对端发送 `SyncPlayerDataMessage` 回滚包；在 `host_only_no_client_capability` 下，卡组检测路径现已降级为 detect-only，避免主机侧半回滚把对局推入 `StateDivergence` |
...
- **最终约束**：只允许房主安装 NCC，因此不能把“客机也装了 NCC 客户端补丁”当成前提。
- **协议事实**：原版 `CombatStateSynchronizer.OnSyncPlayerMessageReceived` 只按 `senderId` 写 `_syncData[senderId]`，`WaitForSync()` 也不会同步 `LocalContext.IsMe(player)`。
- **直接后果**：`SyncPlayerDataMessage` 不是 host-only 安全的客户端回滚通道；即使主机能通过私有 `overrideSenderId` 伪装部分消息，现有 vanilla 消息也不足以完整回滚未装 NCC 客机的本地卡组。
- **2026-04-07 最新调整**：原来的“只改主机权威状态，不发 wire rollback”虽然避免了黑屏，但会在进战斗时把局推入 `StateDivergence`。当前更安全的默认策略已改为 **detect-only**：在没有显式客机能力握手前，只记录并提醒 deck 类作弊，不再对 vanilla 客机做半回滚。
- **未来若要做到真正 host-only 失效**：必须补一条专用控制通道或能力握手，让客机明确消费“本地状态修正/作废”指令，而不是继续尝试复用原版 `SyncPlayerDataMessage`。
