...
2. **生成报告**：`docs/analysis/VC_BASELIB_ANALYSIS_REPORT.md`（7 大模块详解 + 借鉴优先级）
3. **模板字段整理**：`docs/reference/VC_MOD_CHARACTER_TEMPLATE_FIELDS.md`（Mod 角色初始遗物/卡组字段参考）
...
`SimpleModConfig` 配置系统（代码量减少 70%），详细见 `docs/analysis/VC_BASELIB_ANALYSIS_REPORT.md`。
...
### 会话记录 2026-04-07 · MultiplayerTools v0.2.0 发布 + Release 修正

**完成内容：**

1. **MultiplayerTools 安卓适配验证完成**
   - 主菜单首页守卫已落地：悬浮按钮、快捷键、面板本体都只允许在主菜单首页使用
   - 存档扫描已从 PC 专用 `steam/{id}` 扩展为 `steam` / `default` / `editor` 账户目录
   - 移动端当前身份改为读取游戏本体本地玩家 ID；当前测试设备为 `1`
   - 实机验证：向手机 `default/1/.../current_run_mp.save` 下发测试多人档后，游戏可成功读取

2. **MultiplayerTools 版本发布**
   - 版本提升为 `v0.2.0`
   - 作者统一为 `Bilibili@我叫煎包`
   - 主仓库 Release 已创建：
...
3. **主仓库错误 Release 修正**
   - 主仓库最新 Release `v0.4.0` 原标题为 `NoClientCheats & ModListHider Update`
   - 实际资产只有 `ModListHider-v0.3.1.zip`，因此已改为纯 `ModListHider v0.3.1` 描述，去除 NCC 字样
   - 保留原 Release，不删除 tag / 资产

4. **ModListHider 独立仓库补发**
   - `ModListHider` 独立仓库已补发 `v0.3.1`
   - Release 地址：
     `https://github.com/Jianbao233/ModListHider/releases/tag/v0.3.1`
   - 对应源码提交：
     `80370a8 fix: release ModListHider v0.3.1 for Android`

**经验结论：**

- `MultiplayerTools/build.ps1` 只会覆盖游戏目录，不会自动刷新 `torelease/`；发 release 前必须手动同步 DLL
- 安卓端多人档格式与 PC 一致，差异主要在账户目录结构和本地玩家 ID 来源
- GitHub Release 若挂错仓库，至少要先把标题与正文修正，避免资产和项目名不一致
