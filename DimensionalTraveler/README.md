...
《杀戮尖塔 2》角色 Mod 的 `0.1.0` 开发版本。当前已完成完整卡牌阶段，以及首批叙事进度内容：三章 Epoch、全角色事件「未封的记录」、R01/R02 遗物与七位先古的旅人对白分支。后续主要是药剂背包查看 UI、原生药水萃取、R03-R09 遗物、表现资源和真实双进程多人专项。
...
- R01「稳定准则」：每场战斗中第一张成功炼成药剂的配方少消耗 1 点主原理；R02「折叠夹层」：药剂背包容量 `+1`。
...
- 单人自动验收覆盖基础设施、配方支付、生产、品质操作、催化、扩散、回响、实验转化及叙事本地化/注册契约，共 17/17 通过。
...
- 药剂背包查看 UI、原生药水萃取、R03-R09 遗物、美术、动画和 VFX 尚未实现。R01/R02、三章 Epoch、全角色事件「未封的记录」及先古对白分支已实现。
