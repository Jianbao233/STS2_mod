...
《杀戮尖塔 2》角色 Mod 的 `0.1.0` 开发版本。当前已完成完整卡牌、首批叙事进度、R01-R09 遗物、原生药水萃取，以及药剂背包查看与药水弹窗萃取入口的首版 UI。真实双进程攻击药水萃取专项已通过；后续主要是 UI 人工验收与交互调优、其余多人边界、表现资源和发布准备。
...
- 真实双进程攻击药水专项已通过：Host/Client 使用独立 MCP 与相同 run ID，Client 在本地药水栏请求托管注入、展示并提交原生三选一卡，随后双方确认萃取产物、基础／特殊原理增量与 RNG 摘要收敛。
...
- 真实双进程攻击药水萃取已验证，但当前双人自动化尚未覆盖掉线处理、不同队伍规模、杰作轮转远端选牌、扩散／回响跨端结算与长局摘要压力。
