...
public static class AlchemyBackpack
{
    ...
    public static async Task<bool> Purify(AlchemyPotionCard potion, CardModel source)
    {
        if (potion.IsUpgraded || potion.Pile?.Type != PileType)
            return false;

        if (!await AlchemyPrinciples.Spend(potion.Owner, potion.MainPrinciple, 1, source, source))
            return false;
    ...
    public static async Task<AlchemyPotionCard?> Sublimate(AlchemyPotionCard potion, CardModel source)
    {
        if (potion.Quality != PotionQuality.Normal || potion.Pile?.Type != PileType)
            return null;

        if (!await AlchemyPrinciples.Spend(potion.Owner, potion.MainPrinciple, 2, source, source))
            return null;

        return await TransformQuality(potion, PotionQuality.Refined);
    }
        ...
        bool upgraded = false)
    {
        ...
        var destination = HasSpace(player) ? PileType : MegaCrit.Sts2.Core.Entities.Cards.PileType.Hand;
        var result = await CardPileCmd.AddGeneratedCardToCombat(potion, destination, player);
        return result.success ? potion : null;
    }
        ...
        PotionQuality quality)
    {
        ...
        var replacement = scope.CreateCard(PotionCatalog.GetCanonical(potion.Family, quality), potion.Owner);
        ...
        return result?.cardAdded as AlchemyPotionCard;
    }
}