...
> **数据版本**：由 extract_sts2_ids.py 基于 v0.99 反编译（2026-04 前）生成；与当前 v0.110.1 基线的 ID 集合**未核对**（如卡牌数 576 vs 指令参考文档 584 的口径差异）。
> **归档记录**：2026-08-10 移入 `reference/ID数据源/`。策略：游戏更新后若发现模型 ID 不适用，回到此处重新解析并更新本表；更新时同步更新本头注与 `README.md`。
> 中文以官方翻译（zhs）为准。
